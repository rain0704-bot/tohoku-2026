(function () {
  ...
  function dayTemplate(day, options) {
    ...
    const itemsByPeriod = labels.PERIODS.reduce(function (html, period) {
      const items = activeItems(day).filter(function (item) {
        return item.period === period;
      });
      if (!items.length) return html;
      return html + '<section class="period-section"><h3>' + esc(period) + '</h3><div class="timeline">' + items.map(itemTemplate).join("") + "</div></section>";
    }, "");
