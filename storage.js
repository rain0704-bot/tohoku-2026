(function () {
  ...
  function migrateDay3Official(data) {
    ...
    const currentDay3 = data.days[day3Index];
    if (!isPlaceholderDay(currentDay3)) return data;
  ...
  function migrateDay4Official(data) {
    ...
    const currentDay4 = data.days[day4Index];
    if (!isPlaceholderDay(currentDay4)) return data;
  ...
  function isPlaceholderDay(day) {
    if (!day) return false;
    const dayNumber = typeof day.day === "number" ? day.day : "";
    ...
    const expectedPlaceholderTitle = dayNumber ? "Day " + dayNumber + " Placeholder" : "";
    ...
    return (
      (expectedPlaceholderTitle && title === expectedPlaceholderTitle) ||
      title.includes("Placeholder") ||
      route === "待填入" ||
      (driveTime === "待確認" && items.length === 0) ||
      (items.length > 0 && placeholderItems)
    );
  }
